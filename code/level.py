...
class level :
    ...
    def setup(self,tmx_map,level_frames) :
        ...
        for obj in tmx_map.get_layer_by_name("Moving Objects") :
            if obj.name=='spike' :
                spike((obj.x+obj.width/2,obj.y+obj.height/2),level_frames['spike'],(self.all_tile,self.damage_tile),obj.properties['radius'],obj.properties['speed'],obj.properties['start_angle'],obj.properties['end_angle'])
                for i in range(0,obj.properties['radius'],20) :
                    spike((obj.x+obj.width/2,obj.y+obj.height/2),level_frames['spike_chain'],(self.all_tile),i,obj.properties['speed'],obj.properties['start_angle'],obj.properties['end_angle'],Z['background_details'])
            else :
                frames=level_frames[obj.name]
        ...
        for obj in tmx_map.get_layer_by_name("Enemies") :
            if obj.name=='tooth' :
                tooth((obj.x,obj.y),level_frames['tooth'],(self.all_tile,self.damage_tile,self.tooth_tile),self.collision_tile)
            if obj.name=='shell' :
                shell((obj.x,obj.y),level_frames['shell'],(self.all_tile,self.collision_tile),obj.properties['reverse'],self.player)

   # def shoot_bullet(self,pos,direction) :
    #    bullet(pos,(self.all_tile,self.damage_tile,self.bullet_tile),,direction)
