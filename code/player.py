...
class player(pygame.sprite.Sprite) :
    ...
    def check_surface(self) :
        right=pygame.Rect(self.hit_box_rect.topright+Vector2(0,self.hit_box_rect.height/4),(2,self.hit_box_rect.height/2))
        left=pygame.Rect(self.hit_box_rect.topleft+Vector2(-2,self.hit_box_rect.height/4),(2,self.hit_box_rect.height/2))
        ...
        self.is_on_surface['floor']=True if bottom.collidelist(collision_block)>=0 or bottom.collidelist(semi_collision_block)>=0 and self.direction.y>0 else False
        self.platform=None
        sprites=self.collision_tile.sprites()+self.semi_collition_tile.sprites()
        for sprite in [sprite for sprite in sprites if hasattr(sprite,"moving")] :
            if sprite.rect.colliderect(bottom) :
    ...
    def animated(self,fps) :
        ...
        if self.state=='attack' and self.frames_index>=len(self.frames) :
                self.state='idle'
    ...
    def what_state(self) :
        if self.is_on_surface['floor'] :
            ...
            self.state='air_attack' if self.attacking else 'wall' if self.is_on_surface['right'] or self.is_on_surface['left'] else 'jump' if self.direction.y<0 else 'fall'
